...
function intoprogressImgnew(id){
	 ...
	 for(var i=0;i<country.length;i++){
		 if(country[i].id==id){
			 window.location.href="/myvisa/transactVisa/visaProgressImg.html?logintype=5&country="+escape(JSON.stringify(country[i]))+"&countrystatus="+countrystatus+"&orderId="+$.queryString('orderId');
			 return;
		 }
	 }
